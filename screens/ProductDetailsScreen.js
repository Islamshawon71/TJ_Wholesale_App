...
export default function ProductDetailsScreen({ route }) {
    const { productScreen } = route.params;
    ...
    const productImageUrl = `${baseUrl}${productScreen.product_image}`;
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            const existingItem = cartItems.find(
                (item) => item.product === productScreen.product_name && item.variation === (selectedVariation?.id || 'no-variation')
            );
            ...
            addToCart({ ...productScreen, selectedVariation, product_quantity: quantity });
    ...
    return (
        <ScrollView style={styles.scrollView}>
            <SafeAreaView style={styles.container}>
                ...
                <Text style={styles.title}>{productScreen.product_name}</Text>
                <View style={styles.priceContainer}>
                    {width > 600 && (
                        <Text style={[styles.price, styles.strikethrough]}>
                            Regular Price: ${productScreen.regular_price}
                        </Text>
                    )}
                </View>
                <Text style={styles.description}>{productScreen.product_description}</Text>

                <Text style={styles.price}>Price: ${productScreen.sale_price}</Text>
                        ...
                        {variations.map(variation => (
                            ...
                            >
                                <Text style={styles.buttonText}>
                                    Qty: {variation.start_quantity} - {variation.end_quantity} | ${productScreen.sale_price}
                                </Text>
                            </TouchableOpacity>
                        ))}
                    </View>
