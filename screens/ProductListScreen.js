...
export default function ProductListScreen({ productScreen }) {
    const { product_image, product_name, sale_price, regular_price, product_description } = productScreen;
    ...
    const handleProductDetails = () => {
        navigation.navigate('Product Details', { productScreen });
    };

    useEffect(() => {
        const fetchVariations = async () => {
            try {
                const response = await fetch('https://wholesale.techjodo.xyz/api/variations');
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            const existingItem = cartItems.find(item => item.product === product_name);
            ...
            addToCart({ ...productScreen, selectedVariation });
